// Пакет реализует  мультичекер для проведения анализа кода.
// При запуске проверяет файл/файлы следующими анализаторами:
//
// 1. Все стандартые анализаторы пакета golang.org/x/tools/go/analysis/passes:
//appends
...
// 2. Вс анализаторы класса SA пакета staticcheck.io.
//
// 3. Анализатор S1005 пакета staticcheck.io. Drop unnecessary use of the blank identifier
//
// 4. Анализатор ST1005 пакета staticcheck.io.Incorrectly formatted error string
//
// 5. Анализатор bodyclose. Проверяет корректное закрытие res.Body
//
// 6. Анализатор errwrap. Проверяет наличие в fmt.Errorf() директивы отличной от %w
//
// 7. Анализатор OsExitAnalyzer который проверяет, используется ли os.Exit в main пакете.
